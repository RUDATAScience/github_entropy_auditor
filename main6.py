part2のあとに実際の計算実験をいれます。
下記のコードの数理的解説と、検証の中の位置づけを明記しアップロードした結果の考察をmdpi形式のグラフ画像の挿入含めlatex形式で考察ください。（コードスニペット）
コード自体はhttps://github.com/RUDATAScience/github_entropy_auditorにアップロードしますのでciteで引用してください。
import numpy as np
